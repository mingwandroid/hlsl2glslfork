    ...
    Rule {
        ...
        */
        ...
        prepare: {
            ...
            var args = ["-d", "-t", "-v", input.fileName];
            args.push("--output=" + FileInfo.path(input.fileName) + "/Gen_hlslang_tab.c");
            ...
            cmd.src = FileInfo.path(input.fileName) + "/Gen_hlslang_tab.c";
            cmd.dst = output.fileName;
            cmd.sourceCode = function() {
                File.copy(src, dst);
                File.remove(src);
            ...
            cmd.src = FileInfo.path(input.fileName) + "/Gen_hlslang_tab.h";
            cmd.dst = output.fileName;
            cmd.sourceCode = function() {
                File.copy(src, dst);
                File.remove(src);
    ...
    StaticLibrary {
        ...
        */
        ...
        Group {
            ...
            /* Previous failures from flex or bison can leave some intermediate
               parser files in hlslang/MachineIndependent so make sure they are
               excluded as otherwise bad things happen; e.g. compiling C++ code
               with a C compiler and duplicate symbols. */
            excludeFiles: ["hlslang/MachineIndependent/Gen_hlslang_tab.c",
                           "hlslang/MachineIndependent/Gen_hlslang_tab.h",
